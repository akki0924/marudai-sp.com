...
<script>
var genre_ajax_sort_url = "<?= site_dir(); ?>owner/genre/sort_part";
var genre_ajax_del_url = "<?= site_dir(); ?>owner/genre/del_part";
</script>
<script type="text/javascript" src="<?= site_dir(); ?>js/genre_func.js"></script>
...
<input type="button" value="新規追加" class="genre_add_btn">

<ul id="genre_list">
<?php for ( $i = 0, $n = count ($genre_list); $i < $n; $i ++ ) : ?>
  <li id="genre_<?= $genre_list[$i]['id'] ?>" class="ui-state-default">
    <span class="ui-icon ui-icon-arrowthick-2-n-s"></span>
    <?= $genre_list[$i]['name'] ?>
    <input type="button" value="編集" class="genre_edit_btn" data-id="<?= $genre_list[$i]['id'] ?>">
    <input type="button" value="削除" class="genre_del_btn" data-id="<?= $genre_list[$i]['id'] ?>">
  </li>
<?php endfor ; ?>
</ul>
