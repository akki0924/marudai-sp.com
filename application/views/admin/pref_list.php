...
$(function() {
	$('.edit_btn').click(function() {
		$('#id').val( $(this).data('id') );
		$('#operation_form').attr( 'action', '<?= SiteDir(); ?><?= $const['access_admin_dir'] ?>/pref/input' );
	...
	$('#list_area').sortable({
		...
		update: function(event, ui){
			...
			if (
				...
				sortRow = ui.item[0]['sectionRowIndex'];
				var ajaxUrl = '<?= $const['access_admin_dir'] ?>/pref/sort';
				var dataList = {
					'id':sortId,
					'sort_id':sortRow
				};
	...
	<section id="management">
		<div class="container">
			<div class="row mb_80">
				...
				<div class="col6"><a href="<?= SiteDir(); ?><?= $const['access_admin_dir'] ?>/pref" class="btn mng frame">都道府県管理</a></div>
		...
		<div class="container">
		<form method="post" id="operation_form" name="operation_form" action="<?= SiteDir(); ?><?= $const['access_admin_dir'] ?>/pref">
			<h2 class="mb_40">都道府県管理</h2>
			<div class="scroll">
