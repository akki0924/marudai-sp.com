					...
					<form id="operation_form" name="operation_form" action="\<\?= SiteDir(); \?\>admin" method="post" class="white_boad mb_40">
						<div class="max480">
							<table class="mb_20">
								<tbody>
									<tr>
										<th><span class="required"><?= $id_name ?></span></th>
										<td><input type="tel" name="<?= $id ?>" value="\<\?= VarDisp($form['<?= $id ?>']) \?\>" class="\<\?= ((isset($error_account) && $error_account) ? 'formerr_color' : '') \?\>"></td>
									</tr>
									<tr>
									<th><span class="required"><?= $password_name ?>パスワード</span></th>
							...
							<span class="text-danger">\<\?= form_error('<?= $id ?>'); \?\></span><br>
							\<\?php endif; \?\>
