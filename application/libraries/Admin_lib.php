    ...
    public function RegistData ( $registData = '' , $id = '' )
    {
        ...
        if ( is_array ( $registData ) ) {
            ...
            if ( $this->IdExists ( $id, true ) ) {
                ...
                // IDが未セットの場合、IDを生成
                if ( ! isset ( $registData['id'] ) || $registData['id'] == '' ) $registData['id'] = $this->CreateId ();
                // 新規作成
                $returnVal = $this->CI->db_lib->Insert( self::MASTER_TABLE, $registData );
            }
            // 登録成功の場合、IDを返す
            if ( $returnVal ) $returnVal = $registData['id'];
        }
        return $returnVal;
    }
    /*====================================================================
