 ...
 */
class Pref_model extends MY_Model
{
    ...
    public function InputTemplate(bool $validFlg = false) : ?array
    {
        ...
        // 都道府県情報が存在有無情報をセット
        $exists =  $this->pref_lib->IdExists($id);
        $returnVal['exists'] = $exists;
        // 選択情報をセット
        $returnVal['select']['status'] = $this->pref_lib->GetStatusList();
        if ($action == '') {
            // 都道府県ID存在しない
            if (!$exists) {
                // FORM情報をセット
            ...
            // 都道府県IDが存在
                ...
                // 都道府県詳細情報を取得
                $returnVal['form'] = $this->pref_lib->GetDetailValues($id);
            }
        }
        // 遷移アクション時
        else {
            ...
            if ($validFlg) {
                // 各選択情報の表示名をセット
                $returnVal['form']['status_name'] = $this->pref_lib->GetStatusName($returnVal['form']['status']);
            }
        }
    ...
    public function RegistAction($validFlg = false) : ?string
    {
        ...
        if (
            ...
            // 登録処理（IDを返す）
            $returnVal = $this->pref_lib->Regist($form, $id);
        }
    ...
    public function DelAction() : ? bool
    {
        ...
        $id = $this->input->post_get('id', true);
        // 削除処理
        $returnVal = $this->pref_lib->Delete($id);
        // 関連受注情報削除
    ...
    {
        return $this->db_lib->GetCount(pref_lib::MASTER_TABLE, $whereSql);
    }
    ...
    ) : ? array
    {
        ...
        $query = $this->db->query("
            SELECT
                " . pref_lib::MASTER_TABLE . " . id,
                " . pref_lib::MASTER_TABLE . " . name,
                ...
                " . pref_lib::MASTER_TABLE . " . edit_date,
                DATE_FORMAT(" . pref_lib::MASTER_TABLE . ".edit_date, '%Y.%c.%e') AS edit_date_disp
            FROM " . pref_lib::MASTER_TABLE . "
