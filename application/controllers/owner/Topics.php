...
//        $templateVal['footer_tpl'] = $this->load->view ( 'footer', $templateVal, true );
    ...
    public function index()
    {
        ...
        // モデル呼出し
        $this->load->model(Login_model::AUTH_OWNER . '/topics_model');
        
        $templateVal = $this->topics_model->ListTemplate();
        $this->load->view( Login_model::AUTH_OWNER . '/topics_list', $this->sharedTemplate ( $templateVal ));
    }
    // トピックス情報更新処理(AJAX)
    ...
    public function input()
    {
        ...
        // FORM情報をセット
        $form = $this->topics_model->FormInput ();
        ...
        )
        {
            ...
            if ($this->form_validation->run() == FALSE) {
                ...
                // テンプレート読み込み
                $this->load->view ( Login_model::AUTH_OWNER . '/topics_input', self::sharedTemplate ( $templateVal ) );
            }
            else
            {
                // 確認ページ処理
                if ( ! $submit_comp_btn )
                {
                    ...
                    $templateVal = $this->topics_model->InputTemplate();
                    // テンプレート読み込み
                    $this->load->view ( Login_model::AUTH_OWNER . '/topics_conf', self::sharedTemplate ( $templateVal ) );
                }
                // 完了ページ処理
                else
                {
                    ...
                    // 完了ページへ遷移
                    redirect( Login_model::AUTH_OWNER . '/topics/comp' );
                }
            }
        }
        // 入力画面
    ...
    public function comp()
    {
        ...
        $templateVal = array ();
        $this->load->view ( Login_model::AUTH_OWNER . '/topics_comp', self::sharedTemplate ( $templateVal ) );
    }
    // アップロード画像の表示
